...
export function ListingModal({ item, onClose, faved, onFave }: ListingModalProps) {
  ...
  const priceWeek  = (data as any)?.price_per_week  ? parseFloat(String((data as any).price_per_week))  : null;
  const priceMonth = (data as any)?.price_per_month ? parseFloat(String((data as any).price_per_month)) : null;
  const deposit    = (data as any)?.deposit         ? parseFloat(String((data as any).deposit))         : null;
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="lg:w-[58%] flex flex-col">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      ...
                      {(data as any)?.address && <span>· {(data as any).address}</span>}
                    </div>
                  </div>
              ...
              {loadingDetail && (
                <div className="flex items-center justify-center py-6">
                  ...
                  <span className="text-xs text-muted-foreground ml-2">Дэлгэрэнгүй ачаалж байна...</span>
                </div>
              )}
              ...
              {(item.tags || []).length > 0 && (
                <div className="px-5 py-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(item.tags || []).map((tag) => (
                      <span key={tag} className="text-xs px-2.5 py-1 rounded-full bg-primary/8 text-primary border border-primary/15">
                        {tag}
                      </span>
                    ))}
                  </div>
            ...
            <div className="lg:w-[42%] flex flex-col gap-4 p-5 border-l border-border/40 bg-muted/10 overflow-y-auto" style={{ maxHeight: "90vh" }}>
              ...
              <Section title="Үнийн мэдээлэл">
                ...
                {deposit    && <Row label="Барьцаа мөнгө" value={<span className="text-blue-600">{deposit.toLocaleString()}₮</span>} />}
              </Section>
