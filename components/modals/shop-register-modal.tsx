...
export function ShopRegisterModal({ open, onClose }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    tags: "", is_sale: false, seller_name: "", phone: "",
  })
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div className="pointer-events-auto animate-in fade-in slide-in-from-bottom-4 duration-300 w-full max-w-2xl" onClick={e => e.stopPropagation()}>
          <div className="relative w-full max-h-[88vh] flex flex-col bg-background rounded-2xl shadow-2xl border border-border/60">
                  ...
                  <div key={i} className="flex items-center flex-1 min-w-0">
                    <div className="flex flex-col items-center gap-1 shrink-0">
                      ...
                      <span className={cn("text-[10px] whitespace-nowrap hidden sm:block",
                        step === i+1 ? "text-amber-600 font-semibold" : "text-muted-foreground")}>{s}</span>
                    </div>
                    {i < steps.length-1 && (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              ...
              {!submitted && step === 1 && <ImageUploader images={images} onChange={setImages} maxImages={8} />}
              ...
              {!submitted && step === 2 && (
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <input value={form.name} onChange={e => set("name", e.target.value)} placeholder="Кемпинг майхан 4 хүний"
                  ...
                  <div>
                    <label className="text-sm font-semibold mb-1.5 block">Тайлбар <span className="text-destructive">*</span> <span className="text-muted-foreground font-normal text-xs">(10+)</span></label>
                  ...
                  <div>
                    ...
                    <input value={form.tags} onChange={e => set("tags", e.target.value)} placeholder="Хөнгөн, Усны тусгаарлалт..."
                      className="w-full px-4 py-2.5 text-sm bg-muted/50 border border-border/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/30 transition-all" />
                  </div>
                </div>
              )}
              ...
              {!submitted && step === 3 && (
                <div className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="number" value={form.price} onChange={e => set("price", e.target.value)} placeholder="45000"
                  ...
                  <div className="flex items-center gap-3 p-4 rounded-xl bg-muted/40 border border-border/50">
                    <input type="checkbox" id="is_sale" checked={form.is_sale as boolean} onChange={e => set("is_sale", e.target.checked)} className="w-4 h-4 accent-amber-500" />
                    <label htmlFor="is_sale" className="text-sm font-medium cursor-pointer">Хямдралтай бараа гэж тэмдэглэх</label>
                  </div>
              ...
              {!submitted && step === 4 && (
                <div className="space-y-5">
                  <div>
                    ...
                    <input value={form.seller_name} onChange={e => set("seller_name", e.target.value)} placeholder="Таны нэр буюу дэлгүүрийн нэр"
                      className="w-full px-4 py-2.5 text-sm bg-muted/50 border border-border/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/30 transition-all" />
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input value={form.phone} onChange={e => set("phone", e.target.value)} placeholder="9900-0000"
                  ...
                  <div className="p-4 rounded-xl bg-muted/40 border border-border/50 space-y-2">
                    ...
                    <div className="flex gap-1.5 mb-3">
                      {images.slice(0, 5).map((img, i) => (
                        <div key={img.id} className={cn("rounded-lg overflow-hidden bg-muted shrink-0", i === 0 ? "w-16 h-12" : "w-10 h-12")}>
                          <img src={img.preview} alt="" className="w-full h-full object-cover" />
                        </div>
                      ))}
                      {images.length > 5 && (
                        <div className="w-10 h-12 rounded-lg bg-muted flex items-center justify-center text-xs text-muted-foreground font-medium shrink-0">+{images.length - 5}</div>
                      )}
                    </div>
                    {[
            ...
            {!submitted && (
              <div className="flex items-center justify-between px-6 py-4 border-t border-border/50 shrink-0">
                ...
                <Button onClick={() => step < 4 ? setStep(s => (s+1) as Step) : setSubmitted(true)}
                  disabled={!canNext()} className="gap-2 px-6 bg-amber-500 hover:bg-amber-600 text-white">
                  {step === 4 ? <><CheckCircle2 className="h-4 w-4" /> Нийтлэх</> : <>Үргэлжлүүлэх <ChevronRight className="h-4 w-4" /></>}
                </Button>
              </div>
            )}
          </div>
