...
import {
  ...
  ArrowUpDown, Filter, Loader2, Star,
} from "lucide-react";
  ...
  return (
    <div className="absolute top-2.5 left-2.5 z-10 flex items-center gap-1 px-2.5 py-1 rounded-full pointer-events-none"
      style={{
        ...
        boxShadow: "0 2px 8px rgba(245,158,11,0.5), inset 0 1px 0 rgba(255,255,255,0.3)",
      }}>
...
function ListingCard({
  item, view, onFave, faved, onClick,
}: {
  item:    Listing;
  view:    "grid" | "list";
  ...
  return (
    <Card
      onClick={onClick}
      className={cn(
        "group overflow-hidden cursor-pointer transition-all duration-300 bg-background",
        "hover:shadow-xl",
        view === "list" && "flex flex-row h-44",
        item.is_vip
          ? "border-amber-300/70 hover:border-amber-400 dark:border-amber-700/50"
          : "border border-border/60 hover:border-primary/40",
      )}
      style={item.is_vip ? {
        border: "1.5px solid rgba(245,158,11,0.4)",
        boxShadow: "0 0 0 0px rgba(245,158,11,0.15)",
      } : undefined}
    >
      {/* Image */}
      <div className={cn(
        ...
        {/* Favorite */}
        <button
          onClick={e => { e.stopPropagation(); onFave(item.id); }}
      ...
      <CardContent className={cn(
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/50">
          <div className="flex gap-1 flex-wrap">
            ...
            {!item.is_vip && (item.tags || []).slice(0, view === "list" ? 1 : 2).map(tag => (
...
export default function AdsPage() {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      try {
        ...
        setListings(items);
        const maxPrice = Math.max(...items.map(l => parseFloat(String(l.price_per_day))), 0);
  ...
  const toggleLoc  = (loc: string) => setActiveLocations(p => p.includes(loc) ? p.filter(l => l !== loc) : [...p, loc]);
  const clearAll   = () => { setActiveCategories([]); setActiveLocations([]); setPriceMax(priceLimit); setMinRooms(0); setSearch(""); };

  const activeFilterCount =
    activeCategories.length + activeLocations.length +
    (priceMax < priceLimit ? 1 : 0) + (minRooms > 0 ? 1 : 0);
  ...
  const filtered = useMemo(() => {
    ...
    // is_vip field ашиглах
    const vip      = applySort(applyFilters(listings.filter(l => l.is_vip === true)));
    const standard = applySort(applyFilters(listings.filter(l => !l.is_vip)));

    return { vip, standard };
  }, [listings, search, activeCategories, activeLocations, priceMax, minRooms, sortBy]);
                ...
                {(["grid", "list"] as const).map(v => (
                  <button key={v} onClick={() => setView(v)}
                    className={cn("p-2 transition-colors",
                      view === v ? "bg-primary text-primary-foreground" : "bg-muted/60 text-muted-foreground hover:text-foreground")}>
          ...
          <aside className={cn("shrink-0 w-64 space-y-5 transition-all duration-300 lg:block", sidebarOpen ? "block" : "hidden")}>
            <div className="sticky top-40 space-y-6">
              ...
              <div>
                ...
                <input type="range" min={0} max={priceLimit}
                  step={Math.ceil(priceLimit / 100) * 1000}
                  value={priceMax} onChange={e => setPriceMax(Number(e.target.value))}
                  className="w-full accent-primary" />
                <div className="flex justify-between text-xs text-muted-foreground mt-1">
                  <span>0₮</span>
                  <span className="text-primary font-medium">{priceMax.toLocaleString()}₮</span>
                </div>
              </div>
                      ...
                      {filtered.vip.map(item => (
                        <div key={item.id} className="relative">
                          ...
                          <div className="absolute -inset-0.5 rounded-2xl pointer-events-none"
                            style={{
                              background: "linear-gradient(135deg, rgba(251,191,36,0.35), rgba(217,119,6,0.2), rgba(251,191,36,0.35))",
                              filter: "blur(3px)",
                            }} />
                          <div className="relative">
                            <ListingCard
